...
class KITTI3DDataset(Dataset):
    def __init__(self, root_dir, mv3d_split, class_names, sensors, box2d_from_box3d=False, max_num_items=None, calib_file=None):
        self.root_dir = root_dir
        assert mv3d_split in ["train", "val", "trainval", "test", "inference"]
        if "inference" in mv3d_split:
            img_path = os.path.join(self.root_dir, MV3D_SPLIT_KITTI_3D_REMAP["inference"], "image_2")
            self._split = [ele.split(".")[0] for ele in os.listdir(img_path)]
        else:
        ...
        self.calibration_table = self._parse_calibration_files(calib_file)

    def _parse_calibration_files(self, calibration_file=None):
        ...
        if calibration_file is not None:
            calibrations = self._read_calibration_file(os.path.join(self.root_dir, calibration_file))
            per_sample_calibrations = [((sample, calibration[0][1]), calibration[1])
                                       for sample in self._split for calibration in calibrations]
        else:
            calibration_files = [
    ...
    def _get_sample_data(self, sample_id, sensor_name):
        ...
        if sensor_name in ("camera_2", "camera_3"):
            ...
            if "inference" in self._mv3d_split:
                datum['file_name'] = os.path.join(self.root_dir, kitti_3d_split, datum_dir, f'{sample_id}.jpg')
            else:
        ...
        # Labels addition (if applicable)
        labels_path = os.path.join(self.root_dir, MV3D_SPLIT_KITTI_3D_REMAP[self._mv3d_split], "label_2")
...
class KITTI3DMonocularDataset(Dataset):
    def __init__(self, root_dir, mv3d_split, class_names, sensors, box2d_from_box3d, max_num_items, calib_file=None):
        self._kitti_dset = KITTI3DDataset(root_dir, mv3d_split, class_names, sensors, box2d_from_box3d, max_num_items, calib_file)
        self._sensors = sensors
...
def build_monocular_kitti3d_dataset(mv3d_split, root_dir, class_names=VALID_CLASS_NAMES, sensors=('camera_2', ),
                                    box2d_from_box3d=False, max_num_items=None, calib_file=None):
    dataset = KITTI3DMonocularDataset(root_dir, mv3d_split, class_names, sensors, box2d_from_box3d, max_num_items, calib_file)
